...
for train, test in Folds(dset):
    ...
    class Red_Neuronal:
      ...
      def hacia_adelante(self, x):
        ...
        h1 = rampa(self.w1 * x[0] + self.w2 * x[1] + self.w3 * x[2] + self.w4 * x[3] + self.w5 * x[4] + self.w6 * x[5] + self.w7 * x[6] + self.w8 * x[7])
        h2 = rampa(self.w9 * x[0] + self.w10 * x[1] + self.w11 * x[2] + self.w12 * x[3] + self.w13 * x[4] + self.w14 * x[5] + self.w15 * x[6] + self.w16 * x[7])
        o1 = rampa(self.w17 * h1 + self.w18 * h2)
        return o1
      ...
      def entrenamiento(self, datos, clase):
        ...
        for i in range(iteraciones):
          for x, y_real in zip(datos, clase):
            ...
            # Neurona o1
            d_ypred_d_w17 = h1 * rampa(sum_o1)
            d_ypred_d_w18 = h2 * rampa(sum_o1)
            
    
            d_ypred_d_h1 = self.w17 * rampa(sum_o1)
            d_ypred_d_h2 = self.w18 * rampa(sum_o1)
    
            # Neurona h1
            d_h1_d_w1 = x[0] * rampa(sum_h1)
            d_h1_d_w2 = x[1] * rampa(sum_h1)
            d_h1_d_w3 = x[2] * rampa(sum_h1)
            d_h1_d_w4 = x[3] * rampa(sum_h1)
            d_h1_d_w5 = x[4] * rampa(sum_h1)
            d_h1_d_w6 = x[5] * rampa(sum_h1)
            d_h1_d_w7 = x[6] * rampa(sum_h1)
            d_h1_d_w8 = x[7] * rampa(sum_h1)
        
    
            # Neurona h2
            d_h2_d_w9 = x[0] * rampa(sum_h2)
            d_h2_d_w10 = x[1] * rampa(sum_h2)
            d_h2_d_w11 = x[2] * rampa(sum_h2)
            d_h2_d_w12 = x[3] * rampa(sum_h2)
            d_h2_d_w13 = x[4] * rampa(sum_h2)
            d_h2_d_w14 = x[5] * rampa(sum_h2)
            d_h2_d_w15 = x[6] * rampa(sum_h2)
            d_h2_d_w16 = x[7] * rampa(sum_h2)
          ...
          # --- Calcula el error total al final de cada iteración
          if i % 1 == 0:
